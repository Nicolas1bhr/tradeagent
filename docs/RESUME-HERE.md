...
2. **`main` is `137aaa4`, clean, pushed.** Landed 2026-09-07/08, in order, each with its section: `U-loss` `7b46503`, `U-seen-1`
...
3. **Legs in flight at the time of writing (each on its worktree under `~/Projects/ai-trading-software-for-mihael-worktrees/`):**
   ...
   - `u-runner-2` (worktree `U-runner-2`, tip `215cfa4`, built and reported, gate green in its report): being gated by the manager
     for landing; its record is drafted. `u-runner-3` (worktree `U-runner-3`, created on that tip): a fresh builder dispatched 00:05
     2026-09-13 on `docs/briefs/U-runner-3.md` — the backtest, **schema 12**, told to rebase onto `main` before its gate.
...
5. **Briefed, committed, NOT dispatched (no worktree yet):** `docs/briefs/U-api-worker.md` (the app-owned harness: one provider,
   ...
   tool needs the role-and-attempt identity containment puts on `AgentContext`. Schema **13** (`tool_call`; 12 is `U-runner-3`'s) — assign it in the dispatch.
