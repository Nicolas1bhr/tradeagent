...
3. **IN FLIGHT 2026-09-14 evening, each with a fresh builder on its own worktree: (f) `U-flatten-1` (worktree created at `f7c4f31`, no schema, sends
   nothing) and (d) `U-allocator-1` (worktree created at `570279b`, schema 20). Queued as well: `U-promote-bounds` (one item, light, the referee refusing
   promotion without the three bounds — the hole `U-freshness` disclosed), for the next free slot. Queued in `docs/briefs/`, briefed and NOT dispatched — dispatch in this order, at most two heavy legs at once, each on its own worktree
