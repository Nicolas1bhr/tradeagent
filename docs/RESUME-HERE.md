...
6. **Then, in order (the money path, re-planned 2026-09-06 against the vision above; GPT-6 Astra consulted once on
   ...
   (the mission loop, the mission, Guidance, two-press "work on its own") — BOTH LANDED (`U-life` `0ec96c6`, `U-ledger` `640bce5`); runner-red fixers `U-typed-catalog` and
   `U-coid-vanished-win` in flight; (b) `U-meter` built on `u-meter`, carried over the ledger and gated by `U-meter-finish`; (c) `U-loss`: per-trade and daily loss budgets in account currency, enforced by the gateway from the
