...
public class TeardownReachVerifyR8Probes : IDisposable
{
    ...
    public void CONTROL_the_guarded_write_site_cannot_take_the_lease_back()
    {
        ...
        teardown.Stop(steps: () => { }, releaseWitness: witness.Dispose);
    ...
    /// </summary>
    [Fact]
    public void An_unguarded_write_site_takes_the_lease_back_after_the_teardown_released_it()
    {
        var teardown = new AdapterTeardown();
        var witness = new CoidWitness(File_);
        teardown.Started();
        ...
        // The in-flight Place reaches its line 1562. No Record, no flag, no lock.
        witness.Identified("TA-1", "BRK-1");
    ...
    [Fact]
    public void An_unguarded_write_ahead_record_also_takes_the_lease_back_after_the_teardown()
    {
        var teardown = new AdapterTeardown();
        var witness = new CoidWitness(File_);
        teardown.Started();
        ...
        Assert.True(witness.Submitting("TA-2", "SIM", "ES", "Buy", 1m, null),
                    "a stopped strategy could not write — that would be the SAFE outcome");
    ...
    public void A_restart_racing_a_teardown_has_its_witness_released_by_the_old_teardown()
    {
        ...
        teardown.Stop(steps: () => teardown.Started(), releaseWitness: witness.Dispose);
