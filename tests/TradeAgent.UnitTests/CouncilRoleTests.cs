...
public class CouncilRoleTests
{
    ...
    ///
    /// It used to read <c>Assert.Equal(9, Versions.DatabaseSchemaVersion)</c>, which made every later
    /// ADDITIVE migration fail here for no reason of its own — <c>U-report</c> added
    /// <c>mission_event.disposition_detail</c> at 10 and this went red without anything about the
    /// role columns having changed. This is the same move <c>U-wakes</c> made on
    ...
    public void The_role_columns_arrive_at_schema_nine_and_an_unnamed_row_is_the_chairs()
    {
        ...
        Assert.True(Versions.DatabaseSchemaVersion >= 9,
            $"the role columns need schema 9 or later; this build says {Versions.DatabaseSchemaVersion}");
