...
public static class GatewaySchema
{
    ...
    public static object Describe(GatewayStatus? status = null) => new
    {
        ...
        cancel_and_modify_outcomes = "A cancel or a modify is confirmed against the order it named, and only on definite, settled evidence: a cancel is REJECTED — meaning it did not take effect, ask again with a new request_id — when that order is finished, or when it has stayed working and unchanged for a whole grace window. While the platform reports the order as unknown, pending or in flight, your request stays unconfirmed and trading stays paused. A modify is recorded as applied only when the order comes back carrying what you asked for (a price within one tick of the request on the instrument's grid counts), and is never recorded as a definite failure without a refusal from the broker.",
