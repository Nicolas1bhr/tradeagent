...
public static class GatewaySchema
{
    ...
    public static object Describe(GatewayStatus? status = null) => new
    {
        ...
        cancel_and_modify_outcomes = "A cancel or a modify is confirmed against the order it named. If that order turns out to be still working, filled or refused, your cancel is recorded REJECTED — it did not take effect — and you may ask again with a new request_id. A modify is only recorded as applied when the order comes back carrying what you asked for, allowing for the platform rounding a price to the instrument's tick.",
